...
|`ArrivalEntry.departureIndex(): long`|Returns the departure index of the vehicle.<br>The departure index is incremented for every scheduled departure of each individual sidings.<br>It is wrapped back to `1` at 00:00 UTC (For real-time scheduling).<br>A departure index of `0` always represents a manually-departed vehicle.<br>A departure index of `-1` indicates the vehicle has not yet departed.|
