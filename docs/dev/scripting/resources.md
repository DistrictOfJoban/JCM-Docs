...
|`static Resources.hasSystemFont(name: String): boolean`|Returns whether a font family of `name` can be found on the system.<br>Note: Unlike the `getSystemFont` method which special-cases several keywords, this method purely returns the existence of the font family as-is from the user's computer.|
